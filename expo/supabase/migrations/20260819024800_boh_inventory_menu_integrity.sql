...
select i.*,
  ...
  case when i.last_counted is null then 'never_counted'
       when i.last_counted<now()-interval '7 days' then 'stale'
  ...
  case when coalesce(i.on_hand,0)<=0 then 100
       when i.last_counted is null or i.last_counted<now()-interval '7 days' then 90
...
select m.*,
  ...
  case when not m.is_available then 100
       when not (jsonb_typeof(coalesce(m.ingredients,'[]'::jsonb))='array' and jsonb_array_length(coalesce(m.ingredients,'[]'::jsonb))>0) then 20
       else 60 end::integer as inventory_integrity_score
from public.cg_menu_items m;

comment on view public.cg_inventory_intelligence is 'CASPER BOH inventory priority layer. Stale counts and low stock outrank alphabetical display.';
comment on view public.cg_menu_integrity is 'CASPER BOH menu integrity layer. Operator availability is distinct from inventory-verified availability; empty ingredient mappings fail closed as unverified.';
