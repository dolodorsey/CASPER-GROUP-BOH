...
test('BOH inventory ranks operational risk before alphabetical display and expires counts daily',()=>{
  assert.match(migration,/attention_score/);
  assert.match(migration,/last_counted<now\(\)-interval '24 hours'/);
...
test('operator menu availability is distinct from inventory verification and missing mappings are high risk',()=>{
  ...
  assert.match(migration,/then 90/);
